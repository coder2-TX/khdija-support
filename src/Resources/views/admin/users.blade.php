...
<div class="p-4 sm:p-6 max-w-5xl mx-auto space-y-4">
  <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-3 mb-6">
    <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 break-words">
      {{ __('Conversation —') }} {{ $business->name ?? ('#'.$business->id) }}
    </h1>
    <a href="{{ route('admin.conversations') }}"
      ...
      <span>{{ __('Back to Conversations') }}</span>
    </a>
  </div>
        ...
        >
          <div class="flex items-center gap-4 min-w-0">
            @php
              ...
              $initial = strtoupper(mb_substr($it->user?->name ?? 'U',0,1));
            @endphp
            ...
            @php
                $userRole = $it->user?->roles->first()?->name ?? __('Unknown Role');
            @endphp

            <div class="min-w-0">
            <div class="font-semibold text-gray-900">
                {{ $it->user?->name ?? __('Unknown User') }}
              ...
              <div class="text-sm text-gray-500 truncate max-w-[70ch]"
                ...
                {{ $it->last?->body ?? __('No messages yet.') }}
              </div>
            </div>
          </div>
...
setInterval(async () => {
  try{
    const j = await fetchJSON("{{ route('admin.conversations.counters') }}");
  ...
  try{
    ...
    if (j && j.ok && j.item) {
      ...
      if (stream) {
        ...
        stream.dataset.lastId = j.item.id;
        stream.scrollTop = stream.scrollHeight;
      }
